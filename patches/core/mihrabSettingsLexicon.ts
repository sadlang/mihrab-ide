...
// ── عزلُ الاتّجاه (توصيةُ مراجعة تجربة الاستعمال) ──
//
// العنوانُ الواحد يُبنى من مقاطعَ قد تختلف لغاتُها: «المحرّر › Format On Save». وبين
// المقطعين — وفي الواجهة بين الفئة وتسميتها — محارفُ **محايدة** (‏‏'‏›‏'‏، ‏':'‏، الفراغ)
// لا اتّجاهَ لها، فيمنحها خوارزميّةُ bidi اتّجاهَ جارِها الأقوى فتقفز إلى الطرف الخطأ:
// يقرأ المستخدم «‎Format On Save : المحرّر‎» مقلوبةَ الترتيب.
//
// ولا يكفي CSS هنا: النصُّ نفسُه يُسنَد إلى `aria-label` وإلى تلميحات التحويم وإلى
// نتائج البحث، وهي سلاسلُ نصٍّ صِرف لا تبلغها أيّةُ قاعدة. فالعزلُ يجب أن يكون في
// النصّ: ‏FSI (‏U+2068) يفتح مقطعًا **يستنبط اتّجاهَه من محتواه وحده**، وPDI (‏U+2069)
// يغلقه فيعود ما بعده إلى اتّجاه الفقرة. المحرفان بلا عرضٍ ولا صورة.
//
// نعزل كلَّ مقطعٍ لا المعرَّبَ وحده: المقطعُ الإنجليزيّ الباقي هو مصدرُ القفز نفسُه.
// بالهروب الصريح لا بالحرف: التعبيرُ النمطيّ **لا يُهرَّب تلقائيًّا** كما تُهرَّب
// السلاسل، وبناءُ المنبع يُسقِط كلَّ البناء عند أوّل محرفٍ غيرِ ASCII في المخرَج
// المصغَّر. (وقع فعلًا في هذا الملفّ نفسِه — انظر `definiteSingleWord` أعلاه.)
const FSI = '\u2068';
const PDI = '\u2069';
const ISOLATES = /[\u2066-\u2069]/g;

function isolate(segment: string): string {
	// الفارغُ والفراغُ المحض لا يُلَفّان: محرفان بلا محتوًى بينهما لا يعزلان شيئًا،
	// ويبلغان `aria-label` فيقرؤهما بعضُ قارئات الشاشة أو يبلبلان سطرَ برايل.
	return segment.trim() ? FSI + segment + PDI : segment;
}
...
export function arabizeSettingText(text: string): string {
	...
	// نزعُ العزل الوارد أوّلًا: بدونه يلتفّ العزلُ على نفسه عند الاستدعاء المتكرّر
	// فتنكسر الثباتيّة ‏A(A(x)) = A(x)‎ ويتراكم محارفُ لا تُرى في السلسلة.
	const bare = text.replace(ISOLATES, '');

	return bare.split(SEPARATOR).map(segment => {
		// إعداداتُ وسم اللغة تُسبَق بأيقونة ‎'$(bracket) '‎؛ نُبقيها ونعرّب ما بعدها فقط.
		const icon = /^(\$\([^)]*\)\s*)([\s\S]*)$/.exec(segment);
		const done = icon
			? icon[1] + (resolveSegment(icon[2], 0) ?? icon[2])
			: resolveSegment(segment, 0) ?? segment;
		return isolate(done);
	}).join(SEPARATOR);
}
...
export function searchableSettingText(key: string): string {
	// نطاقُ علامات التشكيل العربيّة (‏U+064B…U+0652) والتطويل (‏U+0640) ومحارفُ العزل
	// (‏U+2066…U+2069). نزعُ العزل هنا **إلزاميّ**: البحث يطابق حرفًا بحرف، ومحرفٌ لا
	// يُرى في الكلمة المفتاحيّة يُسقِط المطابقةَ كلَّها بلا أثرٍ ظاهرٍ يدلّ على السبب.
