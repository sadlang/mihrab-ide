    ...
    if (!m || !m.present) {
      ...
      ok(m.startsRight === true, "SC-01/الاتّجاه: الصندوقُ يُصيَّر من اليمين",
        ...
        : `أوّلُ محرفٍ منطقيٍّ يقع يسارَ آخرِه ⇒ الفقرةُ LTR. المرئيُّ «${m.visual}» ` +
          `مقابل المنطقيّ «${m.text}» — textDirection ليست في قائمة السماح (scmInput.ts:310)`);
      ...
      ok(m.lineHeightEm >= INK_FLOOR_EM,
        ...
        `المنبعُ يثبّت Math.round(fontSize*1.5) في scmInput.ts:344`);
      ...
      if (m.ligaEffectEditor === null) {
        ...
        ok(m.features === m.editorFeatures,
          ...
          `قائمة السماح (scmInput.ts:310) فيأخذ الصندوقُ افتراضَ المنبع OFF. ` +
        ...
        ok(Math.abs(m.ligaEffectBox - m.ligaEffectEditor) < 0.01,
          ...
          `fontLigatures ليست في قائمة السماح (scmInput.ts:310)`);
      }
      ...
      if (css) {
        ...
        ok(featFixed, "SC-01/ب: ورقةُ الأنماط تبلغ الأشكالَ السياقيّة في الصندوق",
          "حقنُ font-feature-settings لم يغيّر القيمةَ المحسوبة — لا مخرجَ من الطبقة الثانية");
        if (!boxGrew) {
