...
مخرَجُ الدالّة يلفّ كلَّ مقطعٍ بـFSI…PDI في **النصّ نفسِه** لا بـCSS: النصُّ يُسنَد إلى
`aria-label` (‏settingsTree.ts) وإلى نصّ روابط الوصف، وهي سلاسلُ نصٍّ صِرف لا تبلغها
قاعدةُ نمط. وقاعدةُ mihrab-rtl.css رقم 35 تُخرِج الصنفين من `plaintext` إلى `isolate`
لأنّ الجمعَ بينهما يقلب اتّجاهَ الفقرة — انظر شرحَ القاعدة هناك.

## معلومٌ وغيرُ معالَج (رصدته المراجعتان — لا نُخفيه)

- **اختبارُ المنبع `settingsTreeModels.test.ts` ينكسر**: يتوقّع `'Foo'` ويحصل على
  `'\\u2068Foo\\u2069'`. لا نرقّعه: لا تشغّله سلسلتُنا ولا البناء، ورقعةُ توقّعاتٍ
  فيه مرساةٌ إضافيّةٌ تنجرف. من يشغّل اختباراتِ المنبع يجب أن يعلم أنّ هذا انحرافُنا.
- **مسارٌ ثانٍ بلا تعريب**: الإعدادات التي يساهم بها امتدادٌ بـ`title`/`categoryLabel`
  لا تمرّ بـ`wordifyKey` أصلًا، فتبقى كما كتبها الامتداد — بلا تعريبٍ وبلا عزل.
- **تلميحُ التحويم إنجليزيٌّ خام**: المنبع يبنيه من `setting.key` لا من التسمية.
- **كلفةُ النسخ**: من ينسخ اسمَ إعدادٍ ينسخ معه محرفَي عزلٍ لا يراهما.
- **القاعدة 35 غيرُ مقيسةٍ على عنصرٍ حيّ**: استنباطٌ من UAX #9. القياسُ الحقيقيّ
  (موضعُ النقطتين) في tests/runtime/settings_labels.spec.mjs ويحتاج نسخةً مُطلَقة.
